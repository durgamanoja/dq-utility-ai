...
class DQUtilityMergedStack extends Stack {
  constructor(scope, id, props) {
    ...
    // 2. S3 Bucket for Glue Scripts and Agent Sessions
    ...
    //13. ECS Agent session store bucket (replaces Lambda Agent bucket)
    const ecsAgentSessionStoreBucket = new s3.Bucket(this, 'EcsAgentSessionStore', {
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    ...
    const ecsAgentConstruct = new EcsAgentConstruct(this, 'EcsAgentConstruct', {
      ...
      jwtSignatureSecret: JWT_SIGNATURE_SECRET,
      sessionStoreBucket: ecsAgentSessionStoreBucket,
      cognitoJwksUrl,
    ...
    ecsMcpConstruct.dataBucket.grantReadWrite(existingRole);
    ...
    const athenaWorkgroup = new cdk.aws_athena.CfnWorkGroup(this, 'DQAthenaWorkgroup', {
      ...
      workGroupConfiguration: {
        ...
        resultConfiguration: {
          outputLocation: `s3://${ecsMcpConstruct.dataBucket.bucketName}/athena-results/`,
    ...
    const glueJob = new glue.CfnJob(this, 'AgentGlueJob', {
      ...
      numberOfWorkers: 2,
      defaultArguments: {
        '--TempDir': `s3://${ecsMcpConstruct.dataBucket.bucketName}/tmp/`,
        '--enable-metrics': 'true',
        '--enable-continuous-cloudwatch-log': 'true',
        '--enable-spark-ui': 'false',
        '--sql_query': 'SELECT 1',
        '--output_path': `s3://${ecsMcpConstruct.dataBucket.bucketName}/output/`,
    ...
    ecsMcpConstruct.dataBucket.grantRead(pollerRole);
    // 20. Poller Lambda
